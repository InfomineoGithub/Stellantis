...
async def authenticate(authorization: str | None) -> Auth.types.MinimalUserDict:
    ...
    if os.getenv("BYPASS_AUTH", "").lower() == "true":
        return {"identity": "anonymous", "email": "dev@example.com", "name": "Local Dev User"}
    ...
    try:
        ...
        return {"identity": payload.get("sub") or payload.get("email") or "anonymous", "email": payload.get("email"), "name": payload.get("name")}
